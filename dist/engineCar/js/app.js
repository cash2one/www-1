...
//侧边栏的隐藏与显示
$j(function(){
    var w_h = $j(window).height();//计算视口高度
    var h_h = $j('.js_header').outerHeight();//计算头div高度
    var m_h = Math.round(w_h-h_h);
    var n_h = $j(".js_nav_w").outerHeight();//计算导航部分高度
    ...
    //console.log( $j(".side_on").height());只在第一载入的时候调用所以有bug，到时候需要更改。
});
...
//直接写“0”+对应时间就是了 为什么要写Array(2).join(0)?
//+function(){
//    var time=new Date();
//    console.log(format(time,"d+"))
//}();
...
m1.config(function($stateProvider, $urlRouterProvider) {
    ...
    $stateProvider
        ...
        .state('pro.match_car',{
            url:'/match_car?/mid:role:t',
            views:{
                'content@pro':{
                    templateUrl:'view/pro/t_matchCar_c.html'
                },
                // 'con@pro.match_car':{
                //     templateUrl : 'view/pro/t_allcar.html'
                // },
    ...
    $j.ajaxSetup({
        complete: function(jqXHR, textStatus) {
            ...
            // if (jqXHR.status == '200' && jqXHR.responseText.length <50) {
    ...
    factory.goOut = function(data,success) { // 登出
    ...
    factory.matchCar = function(data,success,error){
        http_get('waji/statis/digMachine/match/daily',data,success,error);
    }
    factory.matchCar2 = function(data,success,error){
        http_get('waji/statis/digMachine/match/scope',data,success,error);
    }
    return factory;
