        ...
        if( $scope.t == "day" ){
            comFun.getDailyDy({'day': $scope.time1, 'managerId': $scope.mid,'page.size': $scope.pageSize, 'page.page': $scope.dydaypageNum},function(data){
                if( data.state == 0){
                    $scope.$apply(function(){
                        ...
                        $scope.$broadcast('dydatachange', data.pageNum, $scope.pageSize, data.totalPage, data.totalElement,$scope.role,$scope.t);
                    });
                }
            });
        }else if( $scope.t == 'period' ){
            comFun.getPeriodDy({'start': $scope.time2, 'end': $scope.time3,'managerId': $scope.mid,'page.size': $scope.pageSize, 'page.page': $scope.dypropageNum},function(data){
                if( data.state == 0){
                    $scope.$apply(function(){
                        $scope.dyList = angular.copy(data.list);
                        $scope.$broadcast('dydatachange', data.pageNum, $scope.pageSize, data.totalPage, data.totalElement);
                    });
                }
            });
        }
            ...
            comFun.getDailyWj({
                ...
                if( data.state == 0){
                    $scope.$apply(function(){
                        ...
                        $scope.$broadcast('wjdatachange', data.pageNum, $scope.pageSize, data.totalPage, data.totalElement);
                    });
                }
            });
        }else if( $scope.t == 'period' ){
            ...
            comFun.getDailyYsc({'day': $scope.time1, 'managerId': $scope.mid ,'page.size': $scope.pageSize, 'page.page': $scope.yscdaypageNum},function(data){
                if( data.state == 0){
                    $scope.$apply(function(){
                        ...
                        $scope.$broadcast('yscdatachange', data.pageNum, $scope.pageSize, data.totalPage, data.totalElement);
                    });
                }
            });
